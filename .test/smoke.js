// NEON EXODUS — headless smoke test, served over localhost so it also runs on CI.
// Browser resolution: BROWSER_PATH env -> bundled puppeteer Chromium -> local Edge/Chrome.
...
async function launchBrowser() {
  ...
  try {
    ...
    // not installed — fall through to auto-detection
  }
  const candidates = [
...
(async () => {
  ...
  // pointer lock fails headless, so we drive the systems directly below
  ...
  const weapons = await page.evaluate(() => {
    ...
    try {
      ...
      out.unlockAgain = !!Weapons.unlock(3);
